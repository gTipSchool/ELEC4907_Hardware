...
def cts_monitor(fpga):
    # MARCH 5 - No longer functions after changed read/write functions to 
    # context manager version. Needs to be verified next time it is used, 
    # but this version is now context managed, just not validated. 
    
    # Should not work because the 
    
    try:
        while True:
            # Check game engine queue status and set flags accordingly
            with fpga.connection as connection:
                if connection.rts and not cts_high.is_set():
                    cts_high.set()
                    logging.info("Raising CTS")
                elif not connection.rts:
        ...
        logging.info("CTS Monitor Shutting Down, No Open Port")
